...
import type { createPropertyScene, ModelView, SceneTheme } from "./property-scene-renderer";
...
export function PropertyScene({ model, variant = "light" }: { model: PropertyModel | null; variant?: SceneTheme }) {
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(async entries => {
      ...
      try {
        ...
        if (cancelled) return;
        controller.current = createPropertyScene(element, model, setLabels, setSnapshot, variant);
        setStatus("ready");
      } catch {
  ...
  if (!model) return <div className={variant === "dark" ? `${styles.unavailable} ${styles.dark}` : styles.unavailable}><strong>Property model unavailable</strong><p>An exact parcel boundary is needed to place the house and roads. No building dimensions or orientation have been assumed.</p></div>;
  const mainInView = model.main?.parts.some(part => part.some((point, i) => i > 0 && clipSegment(part[i - 1], point, model.bounds)));

  const dark = variant === "dark";
  return (
    <div className={dark ? `${styles.modelSection} ${styles.dark}` : styles.modelSection}>
      ...
      {!dark && <div className={styles.modelLegend}>
        ...
        <div className={styles.keyItem}>
          <span className={styles.keyTag}><i className={styles.privateKey} /> <strong>Private line · not drawn</strong></span>
          <span className={styles.keyDetail}>House to sewer main · Homeowner responsibility · No public record of its route</span>
        </div>
        <div className={styles.keyItem}>
...
function PropertyPlan({ model }: { model: PropertyModel }) {
  ...
  return (
    <svg viewBox={`-3 -3 ${width + 6} ${height + 6}`} role="img" aria-label="Recorded parcel and building footprint plan">
      ...
      {/* Recorded sewer main in plan view */}
